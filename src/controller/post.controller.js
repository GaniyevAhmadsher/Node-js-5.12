...
class PostController {
  ...
  async getPostByCategoryController(req, res, next) {
    try {
  ...
  async putPostByIdController(req, res, next) {
    try {
  ...
  async getUserPostsController(req, res, next) {
    try {
      const token = req.headers.authorization?.split(" ")[1];
      if (!token) throw new ApiError("Token invalid or unexist", 401);
  ...
  async searchPostController(req, res, next) {
    try {
      const { q } = req.query;
